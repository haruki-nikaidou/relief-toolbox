<script lang="ts">
	...
	import { GJClient, type GJResult, type StepData, type RatCell } from './gj-client.js';
...
{#snippet matrixDisplay(matrix: RatCell[][])}
	<div class="overflow-x-auto">
		<div class="relative inline-flex items-stretch gap-0 text-sm font-mono">
			<div class="border-foreground w-2 rounded-l-sm border-b-2 border-l-2 border-t-2"></div>
			<div
			...
			>
				{#each matrix as row, rowIdx (rowIdx)}
					{#each row as cell, colIdx (colIdx)}
						<span class="min-w-[2.5rem] text-center">{formatRat(cell.p, cell.q)}</span>
					{/each}
				{/each}
			</div>
			<div class="border-foreground w-2 rounded-r-sm border-b-2 border-r-2 border-t-2"></div>
		</div>
	</div>
{/snippet}
...
<section class="mx-auto max-w-4xl py-4">
	<Card>
		<CardHeader>
			...
			<CardDescription>
				Reduces a matrix to reduced row echelon form (RREF). Supports 2×2 to 10×10 matrices. Enter
				integers, fractions (1/2), or decimals (0.5).
			</CardDescription>
		</CardHeader>
		<CardContent class="space-y-6">
			...
			<div class="flex flex-wrap items-center gap-4">
				<label class="flex items-center gap-2 text-sm font-medium">
					...
					<select
						...
						value={rows}
						onchange={(e) => {
							rows = parseInt((e.target as HTMLSelectElement).value);
						}}
					>
						{#each Array.from({ length: 9 }, (_, i) => i + 2) as n (n)}
							<option value={n} selected={n === rows}>{n}</option>
						{/each}
					</select>
				</label>
				<label class="flex items-center gap-2 text-sm font-medium">
					...
					<select
						...
						value={cols}
						onchange={(e) => {
							cols = parseInt((e.target as HTMLSelectElement).value);
						}}
					>
						{#each Array.from({ length: 9 }, (_, i) => i + 2) as n (n)}
							<option value={n} selected={n === cols}>{n}</option>
						{/each}
					</select>
				</label>
			...
			<div class="overflow-x-auto">
				<div class="relative inline-flex items-stretch gap-0">
					<div class="border-foreground w-3 rounded-l-sm border-b-2 border-l-2 border-t-2"></div>
					<div
					...
					<div class="border-foreground w-3 rounded-r-sm border-b-2 border-r-2 border-t-2"></div>
				</div>
			</div>
